...
function capToFront(s) {
  ...
  //Escribe tu código aquí
let mayus = s.match(/[A-Z]/g).join('');
let minus = s.match(/[a-z]/g).join('');
return mayus + minus;
}
...
function asAmirror(str) {
  ...
  var nuevap = [];
  for (x = 0; x < espejo.length; x++){
    nuevap.push( espejo[x].split("").reverse().join(""))
  }
  return nuevap.join(" ");
} 
